  ...
  export default({
	...
	methods: {
      ...
      setStatus: function(id, status) {
        let self = this;
        let qs = require('qs'); 
        this.$axios.post('/admin/role/set_status/' + id, qs.stringify({status: status}))
      ...
      addSubmit: function(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let qs = require('qs'); 
            let url = this.isEdit ? '/admin/role/update/' + this.curRowId : '/admin/role/create'; 

            this.$axios.post(url, qs.stringify(this.addForm))
            .then(function(response) {
              if (response.data.success) {
                self.$message({
                  message: response.data.msg,
